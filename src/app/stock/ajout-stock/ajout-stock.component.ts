...
/*
 ...
 for (let i=0; i<this.listearticle.length; i++) {
   if(this.codarticle== this.listearticle[i].codearticle){
     this.libelle=this.listearticle[i].libellearticle;
     this.typestockage=this.listearticle[i].typestockage;
     return this.libelle;
   }
   else{
     if(this.codarticle==""){
       ...
       this.libelle="**article inexistant**";
       this.typestockage="**invalide**";
     }
   }
 }
...
//   this.increment();
//   
//   console.log(this.typestockage);
  
//   this.service.addreception(this.nreception, this.nligne1, this.codarticle, this.quantite, this.type, this.fournisseur,this.prixachat,this.datefinv,this.typestockage,this.remise,this.taxe,this.description).subscribe(data3 => {
//     this.stock = data3;
//     console.log(data3);
//     this.stock = JSON.stringify(this.stock);
//     console.log('nreception=' +this.stock[0] + '|' + 'nligne=' +this.stock[1]+ '|' + 'codarticle=' + this.stock[2]+ '|' + 'qte=' + this.stock[3]);
//     this.cleartextbox();
//    // this.dataecrea();
//     this.refreshreception(this.nreception);
...
newreception (){
  var nreception :number; 
  this.service.getnumreception().subscribe(data=>{
    nreception=(data.nreception);
    console.log(nreception);
    this.nreception=nreception;
...
getreception(nreception){
this.service.getrecep(nreception).subscribe(data=>{
this.table1[0]=data;

this.stock=this.table1[0];
},error=> console.log(error));
}
refrech(nreception){
this.stock =[];
this.getreception(nreception); 
}

increment(){
  this.nligne+=1;
}
setnlignzero(){this.nligne=0; }


ajouter(){
  var table3:any=[];
  this.increment();
  console.log(this.nligne);
  console.log(this.fournisseur);
  console.log('reception='+this.nreception+ ' '+ 'article'+this.codarticle);
  this.service.addreception(this.nreception,this.nligne,this.codarticle,this.quantite,this.fournisseur,this.prixachat,this.remise,this.taxe,this.datefinv,this.type,this.description).subscribe(data=>{
    console.log(data);
    table3=data;
   // this.table3=JSON.stringify(table3);
    console.log('reception='+table3[0]+' '+'nligne='+table3[1]+' '+'quntite='+table3[3]);
    this.refrech(this.nreception);
